...
export default async function DateFinderPage({ params }: PageProps<"/[locale]/shraddha-date-finder">) {
  ...
  const underReview = process.env.PANCHANG_REQUIRE_REVIEW === "true";
    ...
    <Container className="py-12 lg:py-16">
      <div className="mx-auto max-w-3xl">
        <header className="no-print">
          ...
          {underReview && <p className="mt-6 rounded-xl border-l-4 border-gold bg-paper p-4">{fd.preliminary}</p>}
        </header>
        <div className="mt-10">
          <DateFinder
            locale={locale}
